...
author: "Autor: Marta Rodríguez Couñago"
date: "Octubre 2020"
output:
  html_document:
    highlight: default
...
* Dado el conjunto de variables, el valor del precio del coche es una variable de la que ya se dispone en el conjunto por lo que al conocer la variable de salida se debería de utilizar un algoritmo de aprendizaje supervisado, ya que disponemos de un histórico en donde comprobar qué tipo de salida debe calcular.


Caso2:

* Si el tipo de la naranja, valor de saldia, pertenece a uno de los valores de entrada, por ejemplo se quiere clasificar según su tamaño, color, acidez..., el valor de salida ya es conocido por lo que se trataría de un algoritmo supervisado, sin embargo si se quiere clasificar según otro valor, por ejemplo distinguiendo los tipos de naranja "Navel", "Blancas" o "Sanguinas" a partir de los valores de entrada se trataría de un algoritmo no supervisado.

Caso 3:

* En este caso ya se conoce previamente un histórico donde aparecen las canciones que los clientes escuchan por lo que también se conoce una respuesta a la pregunta de qué canción le gustaría escuchar después por lo que se trata de un algoritmo de aprendizaje supervisado.
 ...
 * workclass: Tipo de trabajo que realizan

 * education: Tipo de educación que tiene la persona

 * marital-status: Estado civil de la persona

 * occupation: Tipo de trabajo que desempeña

 * relationship: ipo de estado civil de la persona

 * race: Raza, blanco o negro

 * sex: Hombre o mujer

 * hour-per-week: horas de trabajo que realiza a la semana
  
 * native-country: País de origen

 * income: Rango de sueldo en el que se encuentra 

#### Nombre de los atributos

```{r echo=TRUE, message=FALSE, warning=FALSE}

names(datosAdult) <- c("age","workclass","fnlwgt","education","education-num","marital-status","occupation","relationship","race","sex","capital-gain","capital-loss","hour-per-week","nativeCountry","income")

```

#### Discretizado y normalizado de los valores de la tabla

```{r echo=TRUE, message=FALSE, warning=FALSE}
# muestran los primeros 6 valores de cada campo
head(datosAdult)

```

Mostramos estadísticas básicas para comprobar el tipo de los valores y si existen valores nulos.
...
Como se puede ver hay valores nulos para los campos workclass, occupation y native-country por lo que será necesario cambiarlos por otros valores o eliminarlos de la tabla según sea necesario para poder trabajar con los datos.

Al hacer "colSums" se observa que hay demasiadas entradas donde los campos tienen un "?" por lo que si se eliminan se estarían perdiendo demasiados valores y el estudio previo no daría resultados correctos. Los valores serán sustituidos por otros.

Si fueran valores numéricos se tomaría la media global del campo y se sustituiría por esta pero al ser valores de texto se procederá a sustituirlos por "Desconocido"

```{r echo=TRUE, message=FALSE, warning=FALSE}

# Tomamos valor "Desconocido" para los valores vacíos de la variable "country"
datosAdult$workclass[datosAdult$workclass=="?"]="Desconocido"

# Tomamos valor "Desconocido" para los valores vacíos de la variable "country"
datosAdult$occupation[datosAdult$occupation=="?"]="Desconocido"

# eliminamos las filas con valor nulo de native-country
datosAdult$nativeCountry[datosAdult$nativeCountry=="?"]="Desconocido"

# comprobamos que han sido sustituidos correctamente
colSums(datosAdult=="?")

```

##### Discretizado de las variables

Se discretizará el campo edad creando un campo nuevo llamado "segmento_edad" este campo contendrá el rango de edad al que pertenece la persona. La edad se ha separado en intervalos de 10 en 10 siguiendo la siguiente fórmula:

r = numero de intervalos que deseamos establecer
max = numero mas grande, en este caso persona más mayor
min = numero más pequeño, persona más joven
rang = max - min
amplitud = rang / r

```{r echo=TRUE, message=FALSE, warning=FALSE}
# Añadimos una variable nueva a los datos. Este valor es la edad discretizada utilizando el método simple de intervalos de igual amplitud.
summary(datosAdult[,"age"])
# Discretización de la edad
datosAdult["segmento_edad"] <- cut(datosAdult$age, breaks = c(0,10,20,30,40,50,60,70,100), labels = c("0-9", "10-19", "20-29", "30-39","40-49","50-59","60-69","70-79"))
# Observamos los datos discretizados.
head(datosAdult)
# Vemos como se agrupan los datos.
...
Para la normalización se ha utilizado una función que dado un número, el máximo de la columna y el mínimo, calcula su proporción en valores comprendidos entre 0 y 1.

En este caso se normalizará la edad ya que es el valor que se usará más adelante pero se podría normalizar el resto de campos que tengan un valor numérico calculable como por ejemplo un sueldo o horas a la semana ("capital-gain", "capital-loss", "hour-per-week")
...
#Normalizacion de la edad
datosAdult["norm_age"] <- normalizar(datosAdult$age)
...
analizar las relaciones entre las diferentes variables del conjunto de datos para ver si se relacionan y como.

En la primera gráfica se quiere mostrar la relación de los ingresos de una persona con la edad, con esta gráfica se pretende responder a las preguntas: ¿Es cierto que si eres joven tienes un sueldo bajo?

```{r echo=TRUE, message=FALSE, warning=FALSE}

#relacion entre los ingresos y la edad
ggplot(data=datosAdult[1:filas,],aes(x=age,fill=income))+geom_bar()


```

En esta otra se quiere relacionar el sueldo con el genero, para comprobar la brecha de género en los trabajos ya que si eres mujer, por lo general, el sueldo es mas bajo que si eres hombre

```{r echo=TRUE, message=FALSE, warning=FALSE}

#relacion entre los ingresos y el sexo
ggplot(data=datosAdult[1:filas,],aes(x=sex,fill=income))+geom_bar()

```

Para poder visualizar los rangos de edad en los que la mujer y el hombre cobran más.
En este caso hay un porcentaje muy alto de mujeres que a lo largo de su vida se han mantenido por debajo de los 50K y muy pocas de ellas consiguen superar el rango.

```{r echo=TRUE, message=FALSE, warning=FALSE}

ggplot(data = datosAdult[1:filas,],aes(x=sex,fill=income))+geom_bar(position="fill")+facet_wrap(~segmento_edad)

```

Gráfico que muestra la cantidad de mujeres y hombres que ocupan cada puesto y rango de sueldo en el que se encuentran.

```{r echo=TRUE, message=FALSE, warning=FALSE}

ggplot(data = datosAdult[1:filas,],aes(x=sex,fill=income))+geom_bar(position="fill")+facet_wrap(~occupation)
